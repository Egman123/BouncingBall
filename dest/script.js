...
const audio = new Audio("Michael Buble - Holly Jolly Christmas.mp3");
class BouncingBall {
    constructor(x, y) {
        this.res;
        this.x = x;
    ...
    moveBall(deltaTime) {
        ...
        if (this.x + this.radius > (canvas === null || canvas === void 0 ? void 0 : canvas.width) || this.x - this.radius <= 0) {
...
const clickHandler = (e) => {
    audio.play();
    if (balls.length <= 15)
        balls.push(new BouncingBall(e.x, e.y));
};
const drawCanvas = (deltaTime) => {
    if (!canvas || !context)
        return;
...
if (btn) {
    btn.addEventListener("click", () => {
        Debug = !Debug;
        if (Debug)
            btn.innerHTML = "Off Coord";
        else
            btn.innerHTML = "Onn Coord";
    });
}
